...
In production (Cloud Run + A2A + Gemini Enterprise): the user's OAuth
token is injected by Gemini Enterprise into tool_context.state via the
A2A protocol — no request_credential() round-trip is needed.

In local dev (ADK Web UI): the ADK OAuth flow handles consent via
...
def negotiate_creds(tool_context: ToolContext) -> Credentials | dict:
    ...
    # On Cloud Run + A2A + Gemini Enterprise, the token is injected as:
    ...
    # --- Stage 3: Initiate OAuth flow (local ADK Web UI only) ---
    # In production A2A via Gemini Enterprise, the token is always injected
    # in state — if we reach here, either the user hasn't completed consent
    # in Gemini Enterprise or we're running locally in the ADK Web UI.
    # Only call request_credential() if client_id/secret are configured
    # (i.e. local dev); otherwise return an informative error.
    if auths.AUTH_CREDENTIAL.oauth2.client_id and auths.AUTH_CREDENTIAL.oauth2.client_secret:
    ...
    logger.warning("No OAuth token found in session state and no client credentials configured")
    return {
        "status": "error",
        "message": (
            "No OAuth token available. In production, Gemini Enterprise "
            "should inject the token via session state. For local dev, "
