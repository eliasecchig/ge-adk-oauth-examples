...
In local dev (ADK Web UI): AUTH_CONFIG drives the ADK OAuth flow
to prompt the user for consent and exchange the auth code for tokens.

In production (Cloud Run + A2A + Gemini Enterprise): The token is
injected by Gemini Enterprise into the A2A session state — AUTH_CONFIG
is not used, but TOKEN_CACHE_KEY and SCOPES are still referenced by
negotiate_creds().
"""
